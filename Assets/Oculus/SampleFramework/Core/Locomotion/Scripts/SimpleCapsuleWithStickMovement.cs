...
public class SimpleCapsuleWithStickMovement : MonoBehaviour
{
    ...
    void StickMovement()
    {
        ...
        {
            Console.Write("CameraRig or centerEyeAnchor is not assigned!");
            return;
        }

        Quaternion ort = CameraRig.centerEyeAnchor.rotation;
        Vector3 ortEuler = ort.eulerAngles;
        ortEuler.z = ortEuler.x = 0f; // Optional: Set to zero if pitch/roll isn't needed.
        ...
        if (primaryAxis != Vector2.zero)
        {
            ...
            Console.Write($"Move Direction: {moveDir}");
            _rigidbody.MovePosition(_rigidbody.transform.position + moveDir * Speed * Time.fixedDeltaTime);
        }
        else
        {
            Console.Write("No input detected on Primary Thumbstick.");
        }
    }
